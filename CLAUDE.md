...
The job runs on `ubuntu-latest` (no build/test — that is the other job's), `needs: lint-typecheck-test` so a red tree cannot ship, and is gated `if: github.event_name == 'push' && github.ref == 'refs/heads/main'` so PRs — fork PRs especially — never reach the `contents: write` token. That token is granted per job, so `lint-typecheck-test` (which executes third-party code via `uv sync`) stays `contents: read`. Three steps: annotated `git tag -a vX.Y.Z` + push → `gh release create --draft --notes-file … --verify-tag` → `gh release edit --draft=false`. Drafting first means a failure between steps leaves an invisible draft, not a half-formed public release; GitHub auto-marks the newest published release "Latest", so no `--latest`.

Notes come from **`release_notes.py`**, not `--generate-notes`. That flag enumerates merged *pull requests* only, and this repo pushes straight to `main` and has never opened one, so it produced nothing but a compare link no matter what shipped (verified against the `releases/generate-notes` API, which returned the bare link for the 15-commit v0.15.0→v0.16.0 range). The script groups conventional-commit subjects by type instead — hence `fetch-depth: 0` on the release job's checkout, since it reads `git log <previous>..<tag>` locally, and a `previous` output on the `plan` step resolved from `releases/latest` (drafts and prereleases excluded, matching the base `--generate-notes` used). An unrecognised or non-conforming subject is kept **verbatim** under `Other Changes` rather than dropped or de-prefixed; a silently omitted commit understates the release with no signal that it did. This only works while commit subjects stay conventional, which `TestWorkflowBinding.test_repo_history_is_fully_categorised` enforces over the last 40 commits.
...
- `release_notes.py` — builds the body of every GitHub Release from conventional-commit subjects (see Commands › Releases). Stdlib only, so the release job needs no Python setup beyond the runner image's `python3`. `parse_commit` classifies one subject (scope stripped, `!` and `BREAKING CHANGE:` trailers flagged, compound types like `docs+test:` attributed to the first); `render` groups them under `SECTIONS` in declared order, leads with `Breaking Changes` (also listed under their own type, so skimming headings cannot miss one), and closes with a `**Full Changelog**` compare link — or a `/commits/` link when there is no previous tag. Invoked only by `ci.yml`, so `TestWorkflowBinding` binds the two
...
- `tests/test_release_notes.py` — unit tests for `release_notes.py`, the only thing that exercises it (it runs solely inside the release job, where a bad body is visible only after publishing). `TestParseCommit`/`TestRender` cover classification, scope stripping, compound and unknown types, breaking-change markers, section ordering, and both `Full Changelog` link forms; `TestWorkflowBinding` ties `ci.yml` to the script — that it is invoked, passes every required flag, uses `--notes-file` and *not* `--generate-notes`, checks out with `fetch-depth: 0`, exports `previous` from the `plan` step, and that recent commit subjects stay parseable
