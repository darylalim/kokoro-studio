...
- `.github/workflows/release.yml` — tag-triggered release (see Commands › Releases): on a `vX.Y.Z` tag push, verifies the tag matches `pyproject.toml`'s `version`, then `gh release create --generate-notes` on `ubuntu-latest`. `actions/checkout` is SHA-pinned here (not `@v7`) because the job holds `contents: write` and passes `github.token` to `gh`
- `.github/dependabot.yml` — weekly `github-actions` update PRs (`commit-message.prefix: "ci"`). This is the refresh mechanism for pins that cannot float: setup-uv publishes no major tags past v7, and `release.yml` is SHA-pinned. Python dependencies are deliberately **not** covered — those go through `uv add`/`uv lock`
